...
import { retryDeepWikiQuery, retryAllFailedDeepWiki } from "@/lib/deepwiki-channel"
...
export function DeepWikiPanel() {
  ...
  const handleRetryAllFailed = async () => {
    ...
    if (!resolved.deepWiki) return
    const llmConfig = useWikiStore.getState().llmConfig
    void retryAllFailedDeepWiki(project.path, llmConfig, resolved.deepWiki, project.id)
  }
        ...
        <div className="flex items-center gap-1">
          {failedCount > 0 && (
            <Button
              ...
              title={t("deepwiki.retryAll", "Retry all failed")}
            >
              <RefreshCw className="h-3 w-3" />
              {t("deepwiki.retryAll", "Retry all failed")} ({failedCount})
            </Button>
          )}
          <button
